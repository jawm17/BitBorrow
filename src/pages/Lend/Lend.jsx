...
export default function Lend() {
    ...
    useEffect(() => {
        const init = async () => {
            const providerInstance = new BrowserProvider(walletProvider);
            const signerInstance = await providerInstance.getSigner();
            const contractInstance = new ethers.Contract(ContractAddress, MicroloanABI, signerInstance);
    ...
    async function fetchLoans() {
        ...
        const MicroloanContract = new ethers.Contract(ContractAddress, MicroloanABI, provider);
    ...
    async function fundLoan(loanId) {
        let amount = loanRequests[loanId][0];
        const amountInString = amount.toString();
        const parsedAmount = ethers.parseUnits(amountInString, 'wei');

        const tx = await contract.fundLoan(loanId, { value: parsedAmount });
        await tx.wait();
        fetchLoans(contract);
    }
